...
void render()
{
	rect1->render();
	circle1->render();
	btn1->render(mouseX, mouseY, mouseState);
...
void mouse(int button, int state, int wheel, int direction, int x, int y)
{
	...
	if( state == CLICK ) //clicou
	{
	}

	// UI component events
	if (btn1->checkClick(mouseX, mouseY, mouseState))
	{
		btn1->onClick();
	}
}
