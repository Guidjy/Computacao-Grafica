...
void mainMenuInit(Menu*& self, int height, int width)
{
	...
	// insert rect button
	insertRectButton = new Button(Vector2(navBar->getWidth() / 5 * 0 + margin, buttonY), buttonHeight, buttonWidth, 
		"Rectangle", { 0.494, 0.796, 0.580 }, true);
	insertRectButton->setOnClickCallback(drawRect);

	// insert image button
	insertImageButton = new Button(Vector2(navBar->getWidth() / 5 * 1 + margin, buttonY), buttonHeight, buttonWidth, 
		"Image", {0.494, 0.796, 0.580}, true);
	insertImageButton->setOnClickCallback(insertImage);

	// rotate image button
	rotateImageButton = new Button(Vector2(navBar->getWidth() / 5 * 2 + margin, buttonY), buttonHeight, buttonHeight, 
		"", { 0.447, 0.537, 0.855 }, true);
	rotateImageButtonText = new Text(Vector2(navBar->getWidth() / 5 * 2 + margin + buttonHeight + 5, buttonY + buttonHeight / 2), 
		"rotate image", { 1, 1, 1 });
	rotateImageButton->setOnClickCallback(rotateImage);

	// set image grayscale checkbox
	setGrayscaleCheckBox = new CheckBox(Vector2(navBar->getWidth() / 5 * 3 + margin, buttonY), buttonHeight, { 0.447, 0.537, 0.855 }, true);
	setGrayscaleCheckBoxText = new Text(Vector2(navBar->getWidth() / 5 * 3 + margin + buttonHeight + 5, buttonY + buttonHeight / 2), 
		"image grayscale", { 1, 1, 1 });
	setGrayscaleCheckBox->setOnClickCallback(setImageGrayscale);

	// image scale slider
	scaleSlider = new Slider(Vector2(navBar->getWidth() / 5 * 4 + margin, buttonY + buttonHeight / 3), buttonHeight / 4, 
		buttonWidth - margin, { 0.447, 0.537, 0.855 }, true);
	scaleSliderText = new Text(Vector2(navBar->getWidth() / 5 * 4 + margin, buttonY + buttonHeight / 3 + 30), 
		"image scale", { 1, 1, 1 });
